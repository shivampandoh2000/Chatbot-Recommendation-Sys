...
class ChatBot(object):
    ...
    def get_response(self, statement=None, **kwargs):
        ...
        if isinstance(statement, str):
            kwargs['Solution'] = statement

        if isinstance(statement, dict):
            kwargs.update(statement)

        if statement is None and 'Solution' not in kwargs:
            raise self.ChatBotException(
                'Either a statement object or a "Solution" keyword '
        ...
        text = kwargs.pop('Solution')

        input_statement = Statement(Solution=text, **kwargs)
        ...
        if not input_statement.search_text:
            input_statement.search_text = self.storage.tagger.get_text_index_string(input_statement.Solution)

        if not input_statement.search_in_response_to and input_statement.question:
            input_statement.search_in_response_to = self.storage.tagger.get_text_index_string(input_statement.question)
    ...
    def generate_response(self, input_statement, additional_response_selection_parameters=None):
            ...
            if adapter.can_process(input_statement):
                ...
                self.logger.info(
                    '{} selected "{}" as a response with a confidence of {}'.format(
                        adapter.class_name, output.Solution, output.confidence
                    )
                )
        ...
        if len(results) >= 3:
            ...
            for result_option in results:
                result_string = result_option.Solution + ':' + (result_option.question or '')
        ...
        response = Statement(
            ...
            question=input_statement.Solution,
    ...
    def get_latest_response(self, conversation):
        ...
        if latest_statement:
            if latest_statement.question:

                response_statements = list(self.storage.filter(
                    conversation=conversation,
                    text=latest_statement.question,
                    order_by=['id']
                ))
