...
class SQLStorageAdapter(StorageAdapter):
    ...
    def remove(self, statement_text):
        ...
        query = session.query(Statement).filter_by(Solution=statement_text)
        record = query.first()
    ...
    def filter(self, **kwargs):
        ...
        if exclude_text:
            statements = statements.filter(
                ~Statement.Solution.in_(exclude_text)
            )

        if exclude_text_words:
            or_word_query = [
                Statement.Solution.ilike('%' + word + '%') for word in exclude_text_words
            ]
            statements = statements.filter(
                ~or_(*or_word_query)
            )
    ...
    def create(self, **kwargs):
        ...
        if 'search_text' not in kwargs:
            kwargs['search_text'] = self.tagger.get_text_index_string(kwargs['Solution'])
    ...
    def update(self, statement):
        ...
        if statement is not None:
            ...
            if statement.question:
                record.search_in_response_to = self.tagger.get_text_index_string(statement.question)
